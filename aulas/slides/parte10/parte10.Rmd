...
Em outras palavras, se $H_0$ é verdadeira, a v.a. $\chi^2$ segue uma distribuiÃ§Ã£o aproximadamente Qui-quadrado com $k-1$ graus de liberdade.
...
**Valor Crítico**: Para um nível de significÃ¢ncia $\alpha$, encontrar o valor crítico $\chi^2_{crit}$ na tabela Qui-quadrado tal que $P(\chi^2_{k-1} \geq \chi^2_{crit}) = \alpha.$
...
text(xcrit + 5, 0.02, labels= expression('Ã¡rea = P('* chi[k-1]^2 >= chi[crit]^2 *') = '* alpha), pos = 3, cex = 1.2, col = COL[1])
```
</center>
...
Nas situaÃ§Ãµes em que utilizamos os testes de independÃªncia, o esquema de amostragem utizado foi baseado numa amostra aleatÃ³ria de tamanho $n$ que Ã© classificada com respeito a duas caracterÃ???sticas simultaneamente. 

Nesse caso, as frequÃªncias marginais totais (totais por linhas e totais por colunas) sÃ£o variÃ¡veis aleatÃ³rias, pois a cada nova amostragem, nÃ£o temos como saber de antemÃ£o quais serÃ£o os valores dos totais por linhas/colunas.

Se o esquema de amostragem for de dividir a populaÃ§Ã£o em duas subpopulaÃ§Ãµes de acordo com as categorias de uma caracterÃ???stica e selecionar uma amostra de um tamanho prÃ©-determinado para cada subpopulaÃ§Ã£o, entÃ£o esta serÃ¡ uma situaÃ§Ã£o de tabela de contingÃªncia com margens fixas. 

## Teste de Homogeneidade {.build}
Por exemplo, no caso do problema de filiaÃ§Ã£o partidÃ¡ria, poderÃ???amos selecionar amostras aleatÃ³rias de tamanho $200$ entre afiliados do partido $A$ e $300$ dentre os afiliados do partido $B$ e se classificaria essas amostras de acordo com a atitude (favorÃ¡vel, indiferente ou contrÃ¡rio).

O interesse entÃ£o Ã© estudar as proporÃ§Ãµes nessas categorias para determinar se elas sÃ£o aproximadamente iguais para as diferentes subpopulaÃ§Ãµes. Ou seja, queremos testar se as subpopulaÃ§Ãµes sÃ£o homogÃªneas.


## Teste de Homogeneidade {.build .smaller}


Suponha que amostras aleatÃ³rias independentes de tamanho $n_{10}, \ldots, n_{r0}$ sÃ£o selecionadas de $r$ subpopulaÃ§Ãµes $A_1, \ldots, A_r$ respectivamente.  Classificando cada amostra em uma das categorias $B_1, \ldots, B_c$, obtemos uma tabela de contigÃªncia $r \times c$ onde os totais das linhas sÃ£o tamanhos de amostras fixos. 

Tabelas de contingÃªncia $r \times c$ com totais das linhas fixos:
...
As probabilidades das vÃ¡rias categorias de $B$ dentro de cada subpopulaÃ§Ã£o de $A$ tambÃ©m sÃ£o apresentadas a seguir, onde cada $w$ representa uma probabilidade condicional,

$$w_{ij}=P(B_j| A_i)= \text{ probabilidade de } B_j \text{ dentro da populaÃ§Ã£o }  A_i.$$

Probabilidades das categorias de $B$ dentro de cada subpopulaÃ§Ã£o:
...
A hipÃ³tese nula de iqualdade das categorias $B$ para as $r$ subpopulaÃ§Ãµes Ã©:

$$H_0: w_{1j}=w_{2j}= \cdots = w_{rj}, \mbox{ para todo } j=1,2, \ldots c.$$


Sob $H_0$, a probabilidade comum da categoria $B_j$ pode ser estimada do conjunto de amostras notando que de um total de $n$ elementos amostrados, $n_{0j}$ possuem a caracterÃ???stica $B_j$, daÃ??? a probabilidade estimada fica

$$\hat{w}_{1j}=\hat{w}_{2j}= \cdots =\hat{w}_{rj}= \frac{n_{0j}}{n}$$


A frequÃªncia esperada estimada na componente $(i,j)$ sob $H_0$ Ã©:

$E_{ij}=$(NÃºmero de $A_i$ amostrados)$\times$(Probabilidade de $B_j$ dentro de $A_i$)$=n_{i0} \hat{w}_{ij}=\frac{n_{i0}n_{0j}}{n}.$


## Teste de Homogeneidade {.build}

A **estatÃ???stica do teste** Ã© dada por:

$$\chi^2= \sum_{r \times c \text{ componentes }} \frac{(n_{ij}-E_{ij})^2}{E_{ij}}$$
que sob $H_0$ segue uma distribuiÃ§Ã£o $\chi^2$ com $(r-1)\times(c-1)$ graus de liberdade.

Pode-se observar que as fÃ³rmulas e os graus de liberdade dessa seÃ§Ã£o sÃ£o iguais ao da seÃ§Ã£o anterior, somente o mÃ©todo de amostragem e a formalizaÃ§Ã£o da hipÃ³tese nula sÃ£o diferentes.



## Exemplo: Alcoolismo e profissÃµes {.build}

Foi feita uma pesquisa para determinar a incidÃªncia de alcoolismo em diferentes grupos profissionais.   

Separadamente, um amostra aleatÃ³ria entre religiosos, educadores, executivos e comerciantes foi coletada.  
Os dados sÃ£o apresentados na tabela:
...
x = matrix(c(32,268,51,199,67,233,83,267),nrow=4,byrow=TRUE)
 ...
 colnames(x) = c("AlcoÃ³latras","NÃ£o AlcoÃ³latras") 
...
$$w_{ij}=P(B_j| A_i)= \text{ probabilidade de } B_j \text{ dentro da subpopulaÃ§Ã£o } A_i.$$

$H_0: w_{1j}=w_{2j}= \cdots = w_{rj}$, para todo $j=1,2, \ldots c.$

Tabela de contingÃªncia de alcoolismo vs profissÃ£o: frequÃªncia relativa por linha.
...
GrÃ¡fico de barras de alcoolismo vs profissÃ£o: frequÃªncia relativa por linha.

<center>
```{r,echo=FALSE,fig.height=4.5,fig.width=6}
par(mar=c(3, 3, 1, 1))
barplot(t(prop.table(x,1)),xlab="OcupaÃ§Ã£o", main=" ", beside=TRUE,legend.text=TRUE,ylim=c(0,1.1),col=c("lightpink","lightblue"), las=1, cex.axis=1.3,cex.lab=1.3) 
```
</center>
...
Tabela de frequÃªncias esperadas, segundo a hipÃ³tese nula de homogeneidade:
...
Representando por $p_1, p_2, p_3$ e $p_4$ as proporÃ§Ãµes de alcoÃ³latras na subpopulaÃ§Ã£o de religiosos, educadores, executivos e comerciantes, respectivamente, queremos testar a hipÃ³tese:

$$H_0:  p_1= p_2 = p_3 = p_4 \quad \mbox{vs} \quad H_a: \mbox{pelo menos uma proporÃ§Ã£o Ã© diferente}$$ 

A estatÃ???stica observada Ã©:

$$\chi^2_{obs}= \frac{(32-58.25)^2}{58.25}+ \cdots + \frac{(267-282.04)^2}{282.04}=20.59 \stackrel{H_0}{\sim} \chi^2_{`r a$parameter`}$$


Usando o nÃ???vel de significÃ¢ncia $\alpha=`r alpha`$, o $\chi^2$ tabulado Ã© $`r round(chi_crit,2)`$ que Ã© menor do que o observado e portanto a hipÃ³tese nula de homogeneidade Ã© rejeitada.

Como a hipÃ³tese nula foi rejeitada verificamos que hÃ¡ indÃ???cios de que a proporÃ§Ã£o de alcoÃ³latras nas classes profissionais nÃ£o Ã© homogÃªnea. 
...
O Google estÃ¡ constantemente elaborando experimentos para testar novos algoritmos de busca. Por exemplo, o Google pode estar interessado em testar 3 algoritmos usando uma amostra aleatÃ³ria para cada um: `r sum(x[,1])` buscas feitas com o algoritmo atual foram selecionadas ao acaso, `r sum(x[,2])` buscas feitas com o algoritmo teste 1 foram selecionadas ao acaso e `r sum(x[,3])` buscas feitas com o algoritmo teste 2 foram selecionadas ao acaso.

Como avaliar qual o melhor algoritmo? Ã‰ preciso definir alguma medida. 

No caso, o Google irÃ¡ avaliar se o usuÃ¡rio clicou em um dos links da busca e depois nÃ£o realizou uma nova tentativa de busca ou se ele depois realizou nova tentativa (indicando que a primeira busca nÃ£o foi bem sucedida).

Objetivo: 3 algoritmos tÃªm a mesma performance, isto Ã©, a proporÃ§Ã£o de buscas que nÃ£o sÃ£o refeitas Ã© a mesma para os trÃªs algoritmos?
...
A estatÃ???stica $\chi^2$ tem o valor observado de 
...
Usando o nÃ???vel de significÃ¢ncia $\alpha=`r alpha`$, o $\chi^2$ tabulado Ã© $`r round(chi_crit,2)`$ que Ã© maior do que o observado e portanto a hipÃ³tese nula de homogeneidade nÃ£o Ã© rejeitada.
...
* [Ross](http://www.sciencedirect.com/science/article/pii/B9780123743886000132): capÃ???tulo 13. 
* [OpenIntro](https://www.openintro.org/stat/textbook.php): seÃ§Ã£o 6.4
