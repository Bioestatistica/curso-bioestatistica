...
hist(prova1, freq=FALSE, xlab="Notas da Prova 1", ylab="Densidade", 
     main=" ", xlim=c(0,100), ylim=c(0,.5), cex.axis=1.3, cex.lab=1.3, 
     col="lightblue", las=1)
hist(prova2, freq=FALSE, xlab="Notas da Prova 2", ylab="Densidade",
     main=" ", xlim=c(0,10), ylim=c(0,.5), cex.axis=1.3, cex.lab=1.3, 
     col="lightblue", las=1)
```

```{r, echo=FALSE, eval=FALSE, fig.height=3.5, fig.width=8}
library(ggplot2)
set.seed(3)
prova1 <- rnorm(50, mean=70, sd=1)
prova2 <- rnorm(50, mean=7, sd=1)
provas <- data.frame(tipo=rep(c("Prova 1", "Prova 2"), each=50), notas=c(prova1, prova2))
ggplot(data=provas, aes(notas, ..density..)) + 
    geom_histogram(bins=10, colour="black", fill="blue") +
...
> A metade inferior dos dados: 13 observações. A mediana deste subconjunto é $Q_1=x_{(7)}=
2052$ (DF).
...
>
<center>
```{r, echo=FALSE, fig.width=4, fig.height=3.2}
par(mar=c(2, 4, 2, 1))
boxplot(estados, main="População (em 1000 habitantes)", ylab="População", type=2, col="lightblue")
```
</center>
...
## Exemplo: Sódio em cereais matinais {.build .smaller}
...
boxplot(cereais, horizontal=TRUE, outline=TRUE, ylim=c(-1,330), 
        col="lightblue", type=3, xlab="Quantidade de Sódio (mg)")
...
hist(cereais, xlim=c(-1, 330), breaks=seq(0, 320, by=40),
     main="", col="lightblue", xlab="Sódio (mg)", ylab="Frequência", las=1)
```
...
hist(log(bimodalData), nclass=20, xlab="", ylab="Frequência", 
     col="lightblue", main="", xlim=c(-5, 10), las=1)
```
</center>
...
<center>
<img src="figuras/M.png" width=500>
</center>
